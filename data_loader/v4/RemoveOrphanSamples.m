function [Log, IncrState] = RemoveOrphanSamples( Log, IncrState, ETS_id)
    ...
    % don't use the following sampled messages to find the last common sample
    % (even though sampled, they are most likely intermittent, turned on/off during session)
    IgnoreMsg = {'IDLE'};
    ...
    %if ~isempty(IncrState.LastSampleNo)
        ...
        for i = 1 : length(MessageTypes)
            ...
            if (strcmpi(MessageType,IgnoreMsg)), continue;  end
