function Log = AlignSampledData( Log, Incr)
...
% for each message type, what samples are actually valid.
    ...
    for i = 1 : length( MessageTypes)
        ...
        % sample header
        if( isfield( MessageData, 'sample_header') && ~any(strcmpi(MessageType,{'JVR_HOME_POS','JVR_SET_HOME_POS','JVR_TGT_POS','JVR_SET_TGT_POS', 'JVR_HOME_SIZE', 'JVR_SET_HOME_SIZE', 'JVR_TGT_SIZE', 'JVR_SET_TGT_SIZE'}))) 
            ...
            iii = SampleNos <= 0;
            SampleNos(iii) = [];
            if isempty(SampleNos),  continue;   end
    ...
    if( ~isempty( SampledTypes))
        if (isempty(Incr.LastSampleNo))
            FirstSNos = unique( FirstSNos);
            FirstSNo = FirstSNos(1);
        else
            FirstSNo = Incr.LastSampleNo + 1;
        end
    ...
    % refers to the same sample number as i-th column in any other matrix
    ValidSampleMask = [];
