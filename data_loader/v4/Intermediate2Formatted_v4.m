function [Data, IncrState] = Intermediate2Formatted_v4( iData, IncrState)
...
% project
    ...
    INIT_combo_no = InitStartData(IncrState, 'ComboNo', 0 );
    ...
    INIT_jtarget = InitStartData(IncrState, 'TaskJudging.Target', []);
    %
    ...
    num_task_states = length(iData.XM.config.task_state_config.task_end_state{end});
    ...
    if (isfield(D, 'SPM_SPIKECOUNT'))
        ...
        spike_ts = D.SPM_SPIKECOUNT.source_timestamp;
    ...
    numDtSamples = ceil(length(D.SAMPLE_GENERATED.source_timestamp) * 0.01);
    DeltaTimes = diff(D.SAMPLE_GENERATED.source_timestamp(1:numDtSamples));
    dt_per_bin = nanmean(DeltaTimes);
    ...
    % Create Combo Number
    % --------------------------------------------------------------
    target_combo_ids = FillValueAtInds(D.TASK_STATE_CONFIG.target_combo_index(trial_start_idx), ...
                                      trial_start_to_SG.idx, num_samples, INIT_combo_no);
    ...
    % button press masks
    % --------------------------------------------------------------
%     button_sq1_per_bin = FindButtonPressIds(iData, iData.RTMA.defines.PS3_B_SQUARE, 1, INIT_button_sq1);
%     fprintf('#SQ1 button press: %d\n', length(find(button_sq1_per_bin==1)));
% 
%     button_tri1_per_bin = FindButtonPressIds(iData, iData.RTMA.defines.PS3_B_TRIANGLE, 1, INIT_button_tri1);
%     fprintf('#TRI1 button press: %d\n', length(find(button_tri1_per_bin==1)));
%     
%     button_cir1_per_bin = FindButtonPressIds(iData, iData.RTMA.defines.PS3_B_CIRCLE, 1, INIT_button_cir1);
%     fprintf('#CIR1 button press: %d\n', length(find(button_cir1_per_bin==1)));
%     
%     button_x1_per_bin = FindButtonPressIds(iData, iData.RTMA.defines.PS3_B_X, 1, INIT_button_x1);
%     fprintf('#X1 button press: %d\n', length(find(button_x1_per_bin==1)));
    ...
    if (isfield(D, 'PS3_BUTTON_PRESS'))
        ...
        R1_pressed_idx = find(D.PS3_BUTTON_PRESS.whichButton == iData.RTMA.defines.PS3_B_R1);
    ...
    trial_cancelled_idx = [];
    if (isfield(D, 'PS3_BUTTON_PRESS'))
        TRI_pressed_idx = find(D.PS3_BUTTON_PRESS.whichButton == iData.RTMA.defines.PS3_B_TRIANGLE);
        ...
        X_pressed_idx = find(D.PS3_BUTTON_PRESS.whichButton == iData.RTMA.defines.PS3_B_X);
    ...
    % Target
    % --------------------------------------------------------------
    tgts = [D.TASK_STATE_CONFIG.target(1:7,:);D.TASK_STATE_CONFIG.target(31:33,:)];
    target = FillValueAtInds(tgts(:,task_states_to_SG.valid), ...
                                    task_states_to_SG.idx(task_states_to_SG.valid), num_samples, INIT_jtarget);
    Data.TaskJudging.Target = target;
    ...
    if( isfield( D, 'JVR_CURSOR_POS'))
        position.Actual = D.JVR_CURSOR_POS.pos;
        %velocity.Actual = D.ROBOT_CONTROL_SPACE_ACTUAL_STATE.vel;
        position.ActualCoriMatrix = D.JVR_CURSOR_POS.OriMtx;
    end
    ...
    elseif( isfield( D, 'VR_CONTROL_CONFIG'))
        rcc_SG = GetRelativeRefIndices( iData.QL.SequenceNo.VR_CONTROL_CONFIG, ...
                                        iData.QL.SequenceNo.SAMPLE_GENERATED);
                                    
        position.target = FillValueAtInds(D.VR_CONTROL_CONFIG.target(:,rcc_SG.valid), ...
                                          rcc_SG.idx(rcc_SG.valid), num_samples, INIT_position_target);

        ortho_imp = FillValueAtInds(D.VR_CONTROL_CONFIG.orthVelImpedance(:,rcc_SG.valid), ...
                                    rcc_SG.idx(rcc_SG.valid), num_samples, INIT_ortho_imp);

        auto_ctrl = FillValueAtInds(D.VR_CONTROL_CONFIG.autoVelControlFraction(:,rcc_SG.valid), ...
                                    rcc_SG.idx(rcc_SG.valid), num_samples, INIT_auto_ctrl);

        ctrl_dims = FillValueAtInds(D.VR_CONTROL_CONFIG.importantDOF(:,rcc_SG.valid), ...
                                    rcc_SG.idx(rcc_SG.valid), num_samples, INIT_ctrl_dims);
    ...
    position.JointAngles = nan(length(iData.RTMA.MDF.GROBOT_RAW_FEEDBACK.j_ang), num_samples);
...
%     Data.OtherMasks.ButtonPress_X1 = button_x1_per_bin;
    ...
    elseif (isfield(D, 'SPM_FIRINGRATE'))
        Data.FiringRate = active_spike_data; 
    end
