...
% convert to intermediate format
    ...
    RTMADataFile = [basedir 'RTMA_config.mat'];
    ...
    if ~isempty(IncrState.FileNames)
        ...
        XMfiles = cellfun(@(x) sprintf('XM.Trial%s.mat',x), IncrState.FileNames, 'un', false);
        for a = 1:length(XMfiles)
            x = load([basedir XMfiles{a}]);
%            XM(a) = x.XM; 
            XM(a) = x; 
        end
        XM = CatStructFields(XM, 'horizontal', 'merge-fields', 'keep-empty-fields', 'string-concat');
        XM = XM.XM;
        
        %reformat to be easier to access
        XM = StructArray2StructOfCellArrays( XM);

        iData = struct( 'QL', QL, 'RTMA', RTMA, 'XM', XM);
    else
...
%             out.(f{a}) = {in.(f{a})}; %keeping as a cell array prevents having to define which dimension we 'cat' along
            if ~iscell(in.(f{a}))
                out.(f{a}) = num2cell(in.(f{a})); %keeping as a cell array prevents having to define which dimension we 'cat' along
            else
                out.(f{a}) = [in.(f{a})]; %keeping as a cell array prevents having to define which dimension we 'cat' along
            end
        end
