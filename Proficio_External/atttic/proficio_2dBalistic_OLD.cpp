...
#include "/home/robot/Proficio_Balistic_Release/Dragonfly_Message_defs/RTMA_config.h"
//#include "smb://moe/rg2/include/rg2_config.h"
#include "params.h"
...
void moveAstep()
{
  ...
  wam.moveTo(system_center);
        ...
        MDF_PLAY_SOUND ps;
        ps.id = 3;
        CMessage MPS( MT_PLAY_SOUND);
        MPS.SetData( &ps, sizeof(ps) );
        mod.SendMessageDF( &MPS );
        return;
}
...
void ping(barrett::systems::Wam<DOF>& wam, Dragonfly_Module mod)
 {
   try 
   {
     while (true)
     {
       ...
       // Set Task state
       burt_status_data.task_complete = (int)taskComplete;
       burt_status_data.task_success = (int)taskSuccess;
...
int proficio_main(int argc, char** argv,
  ...
  Dragonfly_Module mod( 62, 0); //MID_BURT_ROBOT
  try
  {
    mod.ConnectToMMM("192.168.2.48:7112");//"localhost:7111"); 
  }
  catch( exception &e)
	{
  ...
  while (true) {  // Allow the user to stop and resume with pendant buttons
    ...
    switch(state)
    {
      ...
      {
        //std::cout << "State: " << state << std::endl;
        if (true) //holdTimeT < ((double)(clock() - timer) / CLOCKS_PER_SEC) )
        {
      ...
      case RESET: //NOT SET HERE
      {
        ...
        if (taskComplete) {
          ...
          if (taskSuccess) {
            ...
            CMessage M( MT_GIVE_REWARD );
            M.SetData( &rwd, sizeof(rwd) );
            mod.SendMessageDF( &M );
          } 
        ...
        while (true) // Ensure you pause here to listen for next message
        {
          ...
          if (Consumer_M.msg_type == MT_TASK_STATE_CONFIG)
          {
            ...
            cout << "\n New message" << endl;
            cout << "Distance: " << distance << " : " << task_state_data.distance << endl;
            cout << "Angle: " << angle << " : " << task_state_data.direction <<  endl;
