...
%    DisplayMessageToUser( ['Loading configuration file (' filename')']);
    ...
    if(XM.config.use_prev_sesh_trial_hist(1) == 0 && length(XM.config.use_prev_sesh_trial_hist) > 1)
        error('XM.config.use_prev_sesh_trial_hist should be either 0 or the session numbers to use');
    end
    if(XM.config.do_catch_finding ~= 1 && XM.config.do_catch_finding ~= 0)
        error('XM.config.do_catch_finding must be a 1 or 0');
    end
    if(XM.config.catch_algorithm_on ~= 1 && XM.config.catch_algorithm_on ~= 0)
        error('XM.config.catch_algorithm_on must be a 1 or 0');
    end
    if(XM.config.catch_algorithm_on && ~XM.config.do_catch_finding)
        error('If XM.config.do_catch_finding is 0, XM.config.catch_algorithm_on must also be 0');
    end
