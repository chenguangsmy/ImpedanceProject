...
function RunTaskState( task_state_config, target )
...
if(XM.config.do_video)
  sstring = ['echo R', num2str(XM.rep_num),...
    'T', num2str(XM.trial_num), '-acf', ...
    num2str(task_state_config.auto_command_fraction', '%.5g'),...
    '-vf', num2str(task_state_config.vf_orth_impedance', '%.5g'),...
    '| nc -q 0 ', XM.config.camera_ip, ' ',...
    num2str(XM.config.camera_port)];
