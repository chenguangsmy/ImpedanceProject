...
config = GetStateFromFields(CONFIG, index, NumStates);


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function config = GetStateFromFields(CONFIG, state_idx, NumStates)
...
for i = 1 : length(f)
  ...
  if (strcmp(class(fielddata),'cell') && size(fielddata, 1)>1)
    ...
    for r = 1 : size(fielddata, 1)
      ...
      if (length(list) ~= NumStates)
        % don't try to break up strings that are not
        % "one data per task state"
        save_new_fielddata = false;
        break;
      end
      newdata = vertcat(newdata, list);
    end
    if (save_new_fielddata)
      fielddata = newdata;
    end
  end
  ...
  switch( class( fielddata))
    case 'struct'
      config.(fieldname) = GetStateFromFields(fielddata,...
                                              state_idx,...
                                              NumStates);
    case 'cell'
    ...
    otherwise
      if( size( fielddata, 2) == NumStates)
        % If it is a field that has one element per task state,
        % then pick the value for the specified task state
        config.(fieldname) = fielddata(:,state_idx);
      else
