...
classdef TargetZone
  ...
  methods
    ...
    function setTarget(obj, distance, targetWidth)
      ...
      % set target region
      obj.target_ = Rectangle(tl, bl, br, tr);
      obj.targetR = rotateRectangle(obj.target_, obj.angle_);
      ...
      obj.targetLimR = rotateRectangle(obj.targetLim_, obj.angle_);
    end
    ...
    function rotate(obj, theta)
      ...
      obj.angle_ = theta;
      % Rotate zone
      obj.zoneR = rotateRectangle(obj, obj.zone_, theta);
      % Rotate target
      obj.targetR = rotateRectangle(obj.target_, theta);
    end


    % Determine if a point is within a rectangle (should be private)
    function out = inRegion(rectangle, p)
      V = Point(rectangle.tl, rectangle.bl, rectangle.br, rectangle.tr);
      out = TargetZone.cn_PnPoly(p, V, 4) == 1; % 4 points in rectangle
    end
    ...
    function out = inZone(obj, x,  y,  z)
      p = Point(x, y, z);
      out = TargetZone.inRegion(obj.zoneR, p);
    end
    ...
    function out = cn_PnPoly( P,  V,  n)
      ...
      for i = 1:n   % edge from V[i]  to V[i+1]
         % an upward crossing
        if (((V(i).y <= P.y) && ( V(mod((i+1),n) ).y > P.y)) ...
            || (( V(i).y > P.y ) && ( V(mod((i+1), n)).y <=  P.y))) % a downward crossing
         
          % compute  the actual edge-ray intersect x-coordinate
          vt = (P.y  - V(i).y) / (V(mod((i+1),n)).y - V(i).y);
          % P.x < intersect
          if (P.x <  V(i).x + vt * (V(mod((i+1),n)).x - V(i).x)) 
