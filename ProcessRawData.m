...
%
% To process a single session, start matlab-shell and call ProcessRawData({'Gigantor', 229})
%
% To process rg2 data files in batch mode on hermi:
% > cd rg2/scripts
% > matlab-shell
% > ProcessRawData('BatchProcessRawData.conf')
%
    ...
    elseif (nargin >= 1)
        ...
        [pathstr,env_var,ext] = fileparts(getenv('ROBOTAPP'));
        
        if isempty(env_var)
            error( sprintf(['\nCannot figure out current project from $ROBOTAPP\n']));
        end
        
        if ~strcmpi(project, env_var)
            [ret, hostname] = system('hostname');
        
            if strncmpi(hostname, 'hermi', 5)
                SRC_DIR = strrep(SRC_DIR, env_var, project);
                DATA_DIR = strrep(DATA_DIR, env_var, project);
                PROJ_DIR = strrep(getenv('ROBOTAPP'), env_var, project);
            else
